      ...
      {isOpen && (
        <div className="fixed inset-0 z-100 flex items-center justify-center p-4 sm:p-6">
          ...
          <motion.div
            ...
            className="relative w-full max-w-lg rounded-[2.5rem] shadow-2xl overflow-hidden flex flex-col"
            style={{
              background: "#FFFDF8",
              ...
              <div className="space-y-6">
                ...
                {prices.length > 1 && (
                  <div className="space-y-4">
                    <span className="text-xs font-bold uppercase tracking-widest opacity-80" style={{ color: "#9C8060" }}>
                      ...
                      {prices.map((p, idx) => (
                        <button
                          ...
                          style={selectedPriceIndex === idx
                            ...
                            : {
                              background: "rgba(251,248,243,0.70)",
                              color: "#2C1F0E",
                              border: "1px solid rgba(139,103,38,0.18)"
                            }
                          }
                        >
                          {p}₪
                ...
                <motion.button
                  ...
                  whileTap={{ scale: 0.98 }}
                  onClick={handleAdd}
                  className="w-full py-5 rounded-full text-white font-bold flex items-center justify-center gap-3 text-lg transition-all mt-4"
                  style={{
                    background: "linear-gradient(135deg, #A8832F 0%, #8B6726 60%, #6B4D17 100%)",
                    boxShadow: "0 10px 30px -6px rgba(139,103,38,0.45)"
                  }}
                >
                  <FiShoppingCart size={22} />
                  <span>{t("common.add_to_order") || "إضافة للطلب"}</span>
                  <span style={{ opacity: 0.30 }} className="mx-2">|</span>
